...
const Movie = () => {
    ...
    return (
        <div className={styles.container}>
            <div className={styles.cross}><span onClick={() => history.push('/')} className="material-icons">close</span></div>
            <div className={styles.content}>
                ...
                <div className={styles.genres}>
                    ...
                    <div className={styles.genresNames}>
                        {film.genres ? film.genres.map(data => <span key={data.id}>{data.name}</span>) : null }
                    </div>
                </div>
                <div className={styles.parentalGuide}>
                    <h2>Parental Guide</h2>
                    <div className={styles.guideButtons}>
                        {film.parentalData && film.parentalData.map(data => <button onClick={handleClick} key={data.label} value={data.label}>{data.label}</button> )}
                    </div>
                    <div className={styles.guideDiscription}>
                        <ol>
                            { showGuide ? listContent(showGuide) : null }
                        </ol>
                    </div>
                </div>
                <div className={styles.similarMovies}>
                    ...
                    <div className={styles.similarMoviesCards}>
                        { film.similarMovies && film.similarMovies.map(data => <Card data = {data} key={data.id} />) }
                    </div>
                </div>
            </div>
        </div>
